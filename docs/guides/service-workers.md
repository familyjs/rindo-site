...
[Service workers](https://developers.google.com/web/fundamentals/getting-started/primers/service-workers) are a very powerful api that is essential for PWAs, but can be hard to use. To help with this, we decided to build support for Service Workers into Rindo itself using [Workbox](https://workboxjs.org/).
...
The following example showcases this in combination with the Family framework, but the toast-related code should be easily adaptable to any UI. Add the following to your root component (commonly `app-root.tsx`).
