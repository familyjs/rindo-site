...
To make the global styles available to all the components in your project, the `rindo.config.ts` file comes with an optional [`globalStyle` setting](../config/01-overview.md#globalstyle) that accepts the path to your global stylesheet.
...
:::info
Starting with Rindo v3.0.0, support for IE 11 has been marked as deprecated and will be removed in the next major version of Rindo.
:::
