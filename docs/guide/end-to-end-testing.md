...
it('should create toggle, unchecked by default', async () => {
  ...
  await page.setContent(`
    <nav-toggle class="pretty-toggle"></nav-toggle>
  `);
  const navChange = await page.spyOnEvent('navChange');
  const toggle = await page.find('nav-toggle');
  ...
  expect(navChange).toHaveReceivedEventDetail({
    checked: true,
    value: 'on'
  });
});
