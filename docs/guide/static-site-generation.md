...
Every component, machine and environment will perform differently, so it's difficult to provide a consistent benchmark. However, what we do know is that [Navify's Documentation site](https://navifyframework.web.app/docs) has hundreds of pages and Rindo is able to prerender the entire site in a few seconds.
