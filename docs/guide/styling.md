...
By default, components created with the [`rindo generate` command](cli#rindo-generate-sub-folder) use the shadow DOM.
...
::: warning
**NOTE**: The `:host` pseudo-class selector is used to select the [`Host` element](host-element) of the component
...
To make the global styles available to all the components in your design system, the `rindo.config.ts` file comes with an optional [`globalStyle` setting](config#globalstyle) that accepts the path to your global stylesheet.
