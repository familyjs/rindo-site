---
title: Custom Elements with Rindo
sidebar_label: dist-custom-elements
description: Custom Elements with Rindo
...
This target can be used outside of frameworks as well, if lazy-loading functionality is not required or desired. For using lazily loaded Rindo components, please refer to the [dist output target](./dist.md).
...
| `single-export-module`        | All component and custom element definition helper functions will be exported from the `index.js` file in the output directory. This file can be used as the root module when distributing your component library, see [Publishing](../guides/publishing.md) for more details. |

:::note
At this time, components that do not use JSX cannot be automatically
defined. This is a known limitation of Rindo that users should be aware of.
:::
...
1. Minification will follow what is specified in the [Rindo config](../config/01-overview.md#minifyjs).
...
Ensure that `@rindo/core` is included in your list of dependencies if you set this option to `true`. This is crucial to prevent any runtime errors.
...
:::note
When set to generate type declarations, Rindo respects the export behavior selected via `customElementsExportBehavior` and generates type declarations specific to the content of the generated [output directory's](#dir) `index.js` file.
:::
...
Setting this flag to `true` will cause file minification to follow what is specified in the [Rindo config](../config/01-overview.md#minifyjs). _However_, if [`externalRuntime`](#externalruntime) is enabled, it will override this option and always result in minification being disabled.
