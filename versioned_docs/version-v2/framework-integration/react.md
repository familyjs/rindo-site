---
title: React Integration with Rindo
sidebar_label: React
...
**Supports: React v16.7+ • TypeScript 3.7+ • Rindo v2.9.0+**

Rindo can generate React component wrappers for your web components. This allows your Rindo components to be used within
a React application. The benefits of using Rindo's component wrappers over the standard web components include:
...
wrappers. Your project workspace should contain your Rindo component library and the library for the generated React component wrappers.
...
From the `packages/` directory, run the following commands to create a Rindo component library:
...
Install the `@rindo/react-output-target` dependency to your Rindo component library package.

```bash npm2yarn
# Install dependency
npm install @rindo/react-output-target --save-dev
```

In your project's `rindo.config.ts`, add the `reactOutputTarget` configuration to the `outputTargets` array:
...
export const config: Config = {
  ...
  outputTargets: [
    ...
    reactOutputTarget({
      ...
      proxiesFile: '../react-library/lib/components/rindo-generated/index.ts',
    }),
  ],
};
```
...
```tsx
export * from './components/rindo-generated';
```

### Registering Custom Elements

To register your web components for the lazy-loaded (hydrated) bundle, you'll need to expose a method for registering the underlying Rindo
...
to re-export the Rindo loader's `defineCustomElements()` method. In your React library's entry file (`packages/react-library/lib/index.ts`),
add the following:

```diff
export * from "./components/rindo-generated";
+ export { defineCustomElements } from "rindo-library/loader";
```
...
Before you can successfully build a local version of your React component library, you will need to link the Rindo package to the React package.
...
:::tip
As an alternative to `npm link` , you can also run `npm install` with a relative path to your Rindo component library. This strategy, however, will
...
npm create wite@latest my-app -- --template react-ts
# of if using yarn
yarn create wite my-app --template react-ts
...
This lets you specify component tag names for which you don't want to generate React wrapper components. This is useful if you need to write framework-specific versions of components. For instance, in Family Framework, this is used for routing components - like tabs - so that
Family Framework can integrate better with React Router.
...
[`experimentalImportInjection`](../config/extras.md#experimentalimportinjection) flag on the Rindo config's `extras` object. Once set, this will
require you to rebuild the Rindo component library and the React component library.
